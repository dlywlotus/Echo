...
public class WebSocketController {
    ...
    public void joinLobby(@Payload JoinRoomRequest request,
        ...
        String sessionKey = getSessionKey(simpMessageHeaderAccessor);
        lobbyService.joinQueue(sessionKey, request.username());
    }
    ...
    public void leaveRoom(@DestinationVariable String roomId,
                          SimpMessageHeaderAccessor simpMessageHeaderAccessor) {
        String sessionKey = getSessionKey(simpMessageHeaderAccessor);
        chatRoomService.leaveRoom(sessionKey, roomId);
    }
    ...
    public void sendMessageEvent(@Payload SendMessageRequest request, @DestinationVariable String roomId,
                                 SimpMessageHeaderAccessor simpMessageHeaderAccessor) {
        String sessionKey = getSessionKey(simpMessageHeaderAccessor);
        chatRoomService.sendMessageEvent(sessionKey, request.content(), roomId);
    }
    ...
    public void sendTypingEvent(@Payload SendTypingRequest request, @DestinationVariable String roomId,
                                SimpMessageHeaderAccessor simpMessageHeaderAccessor) {
        String sessionKey = getSessionKey(simpMessageHeaderAccessor);
        chatRoomService.sendTypingEvent(sessionKey, request.isTyping(), roomId);
    }
    ...
    private String getSessionKey(SimpMessageHeaderAccessor simpMessageHeaderAccessor) {
        String sessionId = simpMessageHeaderAccessor.getSessionId();
        if (sessionId == null) {
            throw new WebSocketException("Invalid web socket session id");
        }
        return RedisConstants.SESSION_KEY_PREFIX + sessionId;
    }
}
